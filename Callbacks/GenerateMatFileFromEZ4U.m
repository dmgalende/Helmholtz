...
% (elemInfo), the element to face information (elementFaceInfo), and the 
% connectivity matrices (T) will be created in the current workspace. For 
% the boundary, the connectivity matrices name will have the Tb_attributeName 
% form, and for the submeshes the T_attributeName form. The attributeName 
% will be the same that has been assigned in EZ4U.
%
% NOTE: the elementFaceInfo is a structure with as many fields as
% boundaries. For each boundary, it is a matrix in which the i-row indicates 
% the 2D element (1st column) and the face (2nd column) of that element that 
% contains the 1D element i.
%
...
%      {'matFile1_readed.mat' 'matFile2_readed.mat'...} with the name of 
...
%|                                                                       |
%| Last tested version of Matlab: 8.0.0.783 (R2012b)                     |
        ...
        for iAttrib = 1:nOfAttributes
            ...
            elseif attribType == 1 %Boundaries (2D edges)
                ...
                for item = 1:nOfMarkedElements
                    ...
                    attribInfo{2,indexBoundary,3}(item,:) = [element face];
                end
