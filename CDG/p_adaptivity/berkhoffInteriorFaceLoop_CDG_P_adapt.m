function H = berkhoffInteriorFaceLoop_CDG_P_adapt...
...
% checking mixed faces
face_check = elem_p(infoFaces.interiorFaces(:,1)) ~= elem_p(infoFaces.interiorFaces(:,3));
mixed_faces = interiorFaces(face_check);
omog_faces = interiorFaces(~face_check);

% face mass matrix and shape function for faces of the base elements
M_reference_face = faceMassMatrix(theReferenceElement); % mass matrix of the reference face
Nx_face = zeros(numel(theReferenceElement.IPcoordinates1d),nOfElementNodes,3);
Ny_face = Nx_face ;
for iFace = 1:3
...
%% Base element faces
p = theReferenceElement.degree;
p_faces = omog_faces(elem_p(infoFaces.interiorFaces(omog_faces,1)) == p);
COUNT = 0.1;
time = cputime;
loop_count = 0;
disp(' ')
disp(['Calculating ' num2str(numel(p_faces)) ' base element faces'])
disp(' ')
for i = interiorFaces(p_faces)
    ...
    if loop_count > numel(p_faces)*COUNT
...
%% NON-Mixed faces
for iP = 2:numel(different_p) % I skip the base p
    p = different_p(iP);
    p_faces = omog_faces(elem_p(infoFaces.interiorFaces(omog_faces,1)) == p);
    ...
    loop_count = 0;
    disp(' ')
    disp(['Calculating ' num2str(numel(p_faces)) ' non mixed refined faces P' num2str(p)])
    disp(' ')
    
    % face mass matrix and shape function for faces of the base elements
    M_reference_face = faceMassMatrix(refEl); % mass matrix of the reference face
    ...
    for i = interiorFaces(p_faces)
        ...
        indRef_F = 0.5*sum((elem_p(elem_check(1:iElemF-1))+1).*(elem_p(elem_check(1:iElemF-1))+2))+(1:nOfElementNodes);
        indRef_S = 0.5*sum((elem_p(elem_check(1:iElemS-1))+1).*(elem_p(elem_check(1:iElemS-1))+2))+(1:nOfElementNodes);
        alpha_el = p_adaptivity.ccg(indRef_F);
        sigma_el = p_adaptivity.sigma(indRef_F,:);
        alpha_f = alpha_el(faceNodes(infoFace(2),:));
        sigma_f = sigma_el(faceNodes(infoFace(2),:),:);
        ...
        % calculate n1 (=-n2)
        [n,A] = faceNormalArea(infoFace,refEl,X,T);
        M_iFace = M_reference_face * A/2; % mass matrix of the current face (/2 because the lenght of the reference face is 2)
        ...
        % first element
        [FA11, FA12, FA21, FA22] = lifting(1,refEl,n,infoFace,R_ElemF,M_iFace,C12);

        % second element
        [SA11, SA12, SA21, SA22] = lifting(2,refEl,-n,infoFace,R_ElemS,M_iFace,-C12);

        % Face contribution
        [XX,XY,YX,YY] = faceContribution(refEl,Xe_F,Xe_S,n,A,infoFace,alpha_f,C11,C12,sigma_f,omega,...
            Nx_face,Ny_face);
...
%% Mixed faces
disp(' ')
disp(['Calculating ' num2str(nnz(mixed_faces)) ' mixed faces'])
disp(' ')
COUNT = 0.1;
time = cputime;
loop_count = 0;
for i = interiorFaces(mixed_faces)
    loop_count = loop_count+1;
    infoFace = infoFaces.interiorFaces(i,:);
    ...
    % elements
    M_reference_face_F = p_adaptivity.fSP.(['P' num2str(refEl_F.degree)]).faceMassMatrix;
    M_reference_face_S = p_adaptivity.fSP.(['P' num2str(refEl_S.degree)]).faceMassMatrix;
    M_reference_face_X = ...
        p_adaptivity.fSP.(['P' num2str(refEl_F.degree)]).(['faceMassMatrix_x_p' num2str(refEl_S.degree)]);
    N_face_F = refEl_F.N1d;
    N_face_S = p_adaptivity.fSP.(['P' num2str(refEl_S.degree)]).(['N_face_x_p' num2str(refEl_F.degree)]);
    Nx_face_F = p_adaptivity.fSP.(['P' num2str(refEl_F.degree)]).Nx_face(:,:,iFaceF);
    Ny_face_F = p_adaptivity.fSP.(['P' num2str(refEl_F.degree)]).Ny_face(:,:,iFaceF);
    Nx_face_S = p_adaptivity.fSP.(['P' num2str(refEl_S.degree)]).(['Nx_face_x_p' num2str(refEl_F.degree)])(:,:,iFaceS);
    Ny_face_S = p_adaptivity.fSP.(['P' num2str(refEl_S.degree)]).(['Ny_face_x_p' num2str(refEl_F.degree)])(:,:,iFaceS);
    ...
    M_face_12 = M_reference_face_X * A/2;
    ...
    if loop_count > nnz(mixed_faces)*COUNT
        disp(['Calculated: ' num2str(100*COUNT) '% ; time elapsed ' num2str(cputime-time)])
        time = cputime;
        COUNT = COUNT + 0.1;
    end
