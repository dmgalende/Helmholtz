function dampingMatrix = berkhoffDampingMatrix_CDG_P_adapt(X,T,theReferenceElement,infoFaces,k,alpha,p_adaptivity)
...
for i = faces(face_check)
    ...
    ind = 0.5*sum((elem_p(1:iElem-1)+1).*(elem_p(1:iElem-1)+2))+1 : ...
        0.5*sum((elem_p(1:iElem)+1).*(elem_p(1:iElem)+2));
    dm_el = elementalDampinMatrix(Xe,refEl,infoFace,ke,alpha_el,nOfElementNodes);
    ind_transp = transpose(ind);
...
dampingMatrix = sparse(I(I~=0),J(I~=0),Kr(I~=0)+sqrt(-1)*Ki(I~=0),dim,dim);
