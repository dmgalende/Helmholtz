function f = berkhoffReflectingVector(X,T,theReferenceElement,u0,u0grad,k,ccg,alpha)
...
for ielem = 1:nOfElements
 ...
 fe = forceElementalVector(Xe,theReferenceElement,u0e,u0grade,ke,ccge,alpha);
 f(Te)=f(Te)+fe;
 clear fe;
end

%______________________________________________________________
function fe = forceElementalVector(Xe,theReferenceElement,u0e,u0grade,ke,ccge,alpha)
...
for g = 1:ngauss
  ...
  fe = fe + ccg*N_g'*(n_g_x*phi0der_g(1) ...
      + n_g_y*phi0der_g(2) - sqrt(-1)*k*alpha*phi0_g)*dline;
end
