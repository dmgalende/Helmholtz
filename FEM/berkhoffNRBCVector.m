function f = berkhoffNRBCVector(X,T,theReferenceElement,u0,u0grad,k,ccg,sigma,omega)
...
for ielem = 1:nOfElements
 ...
 fe = forceElementalVector(Xe,theReferenceElement,u0e,u0grade,ke,ccge,sigmae,omega);
 f(Te)=f(Te)+fe;
 clear fe;
end

%______________________________________________________________
function fe = forceElementalVector(Xe,theReferenceElement,u0e,u0grade,ke,ccge,sigmae,omega)
...
for g = 1:ngauss
  ...
  fe = fe + ccg*N_g'*(n_g_x*(param(2)/param(1))*phi0der_g(1) ...
      + n_g_y*(param(1)/param(2))*phi0der_g(2) - sqrt(-1)*k*phi0_g)*dline;
end
