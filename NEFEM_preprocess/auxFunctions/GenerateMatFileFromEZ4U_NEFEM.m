function varargout = GenerateMatFileFromEZ4U_NEFEM(arg1,arg2,arg3,arg4,arg5)

% This is the same function that GenerateMatFileFromEZ4U but the boundary
% connectivity matrices. They include the element and the face. Elements 
% with more than one face on a boundary display warnings. The 1D 
% numbering is ignored. Only works with 2D triangles.
...
for ifile = 1:nOfMeshFiles
    ...
    repeatedFaceInfo = false(nOfElements,1);
    ...
    if nOfAttributes
        ...
        for iAttrib = 1:nOfAttributes
            ...
            elseif attribType == 1 %Boundaries (2D edges)
                ...
                attribInfo{2,indexBoundary,1} = zeros(nOfMarkedElements,2);
                for item = 1:nOfMarkedElements
                    lineVar = fscanf(fid,'%d',3);
                    element = lineVar(1);
                    face = lineVar(2);
                    if repeatedFaceInfo(element,1)
                        disp(['---> Element ' num2str(element) ' has more than one '...
                            'face on the boundary (elem ' num2str(item) ' in boundary '...
                            num2str(attributeName) ')'])
                    else
                        repeatedFaceInfo(element,1) = true;
                    end
                    attribInfo{2,indexBoundary,1}(item,:) = [element face];
                end
    ...
    elemInfo = struct('type',elemType,'nOfNodes',nOfElementNodes,...
        'faceNodes',elemFaceNodes);
        
    %Close EZ4U mesh file
    fclose(fid);
    
    %Creating .mat file
    matFileName = [ez4uFile(1:end-4) '_readed.mat'];
