function [error eg domArea] = calculateL2ErrorTwoSolutionNested(data_1,data_2,option_solution,ruled)

% calculate the L2 error element by element for two meshes with different p

if size(data_1.mesh.T,1) > size(data_2.mesh.T,1)
    data_C = data_1;
    data_M = data_2;
else
    data_C = data_2;
...
% incident potential
if isempty(data_M.ip.waveNumberValue)
    if data_M.ip.waveNumberBoundary
        nameConBoundary = data_M.mesh.fieldNames{data_M.mesh.boundaryIndex(...
            data_M.ip.waveNumberBoundary)};
        nodesBoundary = unique(data_M.mesh.(nameConBoundary));
        data_M.ip.waveNumberValue = data_M.bottom.waveNumber(nodesBoundary(1));
    end
end
angle = data_M.ip.direction*(pi/180);
kvector = data_M.ip.waveNumberValue*[cos(angle) sin(angle)];
...
if ruled
    % ruled connectivity
    rul_M = createRuledConnectivity(X_M,T_M);
    rul_C = createRuledConnectivity(X_C,T_C);
else
    % ruled connectivity
    rul_M = false(nElem_M,1);
    rul_C = false(nElem_C,1);
    % out of PML elements
    OutOfPMLelements = data_M.OutOfPMLelements;
    OutOfPMLelements_C = createOutOfPMLConnectivity(data_C);
    rul_M(OutOfPMLelements) = true;
    rul_C(OutOfPMLelements_C) = true;
end

% elements to compute
elements = 1:nElem_M;
elements2compute = elements(rul_M);
enlacMap_C = find(rul_C);

% cross connectivity
[C Rot] = createCrossConnectivity(X_M,T_M(rul_M,:),X_C,T_C(rul_C,:),data_M.mesh.referenceElement.faceNodes);

% number of nodes per element
nOfNodes_M = size(data_M.mesh.referenceElement.NodesCoord,1); % N of nodes mother mesh
nOfNodes_C = size(data_C.mesh.referenceElement.NodesCoord,1); % N of nodes child mesh

% Compute shape functions at Gauss points of the child mesh
coordGauss = data_C.mesh.referenceElement.IPcoordinates;  % Gauss points child mesh
coordRef = data_M.mesh.referenceElement.NodesCoord;       % Nodes on the mother mesh
nOfGauss = size(coordGauss,1);
nDeg = data_M.mesh.referenceElement.degree;
...
% computation
computation_M = data_M.computation;
computation_C = data_C.computation;

%free memory
referenceElement_C = data_C.mesh.referenceElement;
nDeg_C = referenceElement_C.degree;
clear data_C data_M
loop_ind = 0;
...
% allocate vectors
error = zeros(nElem_M,1);
solNorm = zeros(nElem_M,1);
domArea = ones(nElem_M,1);
if any(strcmp(computation_M,{'CDG','DG'})) % CDG or DG solution

    if any(strcmp(computation_C,{'CDG','DG'})) % CDG or DG  % CDG reference solution
        % Loop in elements
        for ielem = 1:numel(elements2compute)
            loop_ind = loop_ind+1;
            iElem = elements2compute(ielem);
            Ce_M = C(ielem,:);
            Rote_M = Rot(ielem,:);
            Te_M = T_M(iElem,:);
            Xe_M = X_M(Te_M,:);
            ...
            for n = 1:nestLev
                ...
                ue_C = u_C(ind_C);
                [error_n solNorm_n area_n] = calculateElementalError(ue_C,ue_M,Xe_C,Xe_M,referenceElement_C,...
                    kvector,shapeFunctions_cell{n},rot,coordGauss_map_cell{n},nDeg,...
                    coordRef,option_solution);
    ...
    elseif strcmp(computation_C,'FEM') % FEM reference solution
        % Loop in elements
        for ielem = 1:numel(elements2compute)
            loop_ind = loop_ind+1;
            iElem = elements2compute(ielem);
    ...
    if strcmp(computation_C,'CDG')  % CDG reference solution
        % Loop in elements
        for ielem = 1:numel(elements2compute)
            loop_ind = loop_ind+1;
            iElem = elements2compute(ielem);
...
function [elemError solNorm elemArea] = calculateElementalError...
    (ue_C,ue_M,Xe_C,Xe_M,referenceElement,kvector,n,rot,gauss_map_M,nDeg,coordRef,optSol)
...
if strcmpi(optSol,'FA')
    ip_gauss = transpose(exp(sqrt(-1)*kvector*xyg_C'));
    Ue = abs(Ue+ip_gauss);
    ue = abs(ue+ip_gauss);
end
err_gauss = Ue-ue;
